...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is SquarePlayerWallNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SquarePlayerWallNetworkObject.Length > 0 && SquarePlayerWallNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SquarePlayerWallNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		public CharSquareBehavior InstantiateCharSquareNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			...
			var netBehavior = go.GetComponent<CharSquareBehavior>();
		...
		[Obsolete("Use InstantiateSquarePlayerWall instead, its shorter and easier to type out ;)")]
		public SquarePlayerWallBehavior InstantiateSquarePlayerWallNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SquarePlayerWallNetworkObject[index]);
			var netBehavior = go.GetComponent<SquarePlayerWallBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SquarePlayerWallBehavior>().networkObject = (SquarePlayerWallNetworkObject)obj;
		...
		public SquarePlayerWallBehavior InstantiateSquarePlayerWall(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SquarePlayerWallNetworkObject[index]);
			var netBehavior = go.GetComponent<SquarePlayerWallBehavior>();
